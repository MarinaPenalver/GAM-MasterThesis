...
However, ordinary cross-validation, which involves fitting the model $n$ times, can be computationally costly. Therefore, \textit{generalized cross-validation} emerges as a computationally efficient alternative, effectively selecting the value of $\lambda$ that optimizes the balance between fit and smoothness.
...
y_pred <- predict(smooth_spline_model, x_pred)$y

data_spline <- data.frame(x_pred, y = y_pred)
```
...
ggplot(data_spline, aes(x = x)) +
  ...
  geom_line(aes(x = x_pred, y = y_pred, color = "Prediction"), linewidth = 1) +
...
x_pred <- seq(min(x), max(x), length.out = 300)
y_pred1 <- predict(splineS1, x_pred)$y
y_pred2 <- predict(splineS2, x_pred)$y
y_pred3 <- predict(splineS3, x_pred)$y


data_spline <- data.frame(x_pred, y_pred1, y_pred2, y_pred3)
```
...
ggplot(data_spline, aes(x = x)) +
  geom_line(aes(x = x_pred, y = y_pred1, color = "1"), linewidth = 1) +
  geom_line(aes(x = x_pred, y = y_pred2, color = "2"), linewidth = 1) +
  geom_line(aes(x = x_pred, y = y_pred3, color = "3"), linewidth = 1) +
